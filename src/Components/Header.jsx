...
const Header = () => {
  ...
  return (
    <div className='w-full h-20 bg-[#334238] flex justify-between items-center px-10'>
      ...
      <div className='md:hidden z-50 cursor-pointer' onClick={toggleMenu}>
        ...
        {mobileMenu && (
            <ul className='absolute top-20 right-4 w-[75vw] bg-[#334238]shadow-lg rounded p-6 flex flex-col gap-4 z-40 md:hidden'>
                {navItems.map((item,index) => (
                    <li
                    key={index} 
                    onClick={() => handleNavigate(item.path)}
